// ── Analytics ────────────────────────────────────────────────────────────────
const _sid=(()=>{let s=localStorage.getItem('footio_sid');if(!s){s=crypto.randomUUID();localStorage.setItem('footio_sid',s);}return s;})();
const _evQ=[];let _evT=null;
function track(event,data={}){_evQ.push({event,data});clearTimeout(_evT);_evT=setTimeout(_flush,2000);}
async function _flush(){if(!_evQ.length)return;const batch=_evQ.splice(0,50);try{await fetch('/api/events',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_id:_sid,user_id:authUser?.id??null,referrer:document.referrer||null,events:batch})});}catch(e){}}
// ── End Analytics ────────────────────────────────────────────────────────────
...
let _gameStart=0;
function markPlayed(game,won){const played=JSON.parse(localStorage.getItem('played')||'{}');played[game]={d:getToday(),w:won};localStorage.setItem('played',JSON.stringify(played));const dur=_gameStart?Math.round((Date.now()-_gameStart)/1000):null;track(won?'game_won':'game_lost',{game,duration_s:dur});}
...
async function loadLeaderboard(){track('leaderboard_view');try{const [lbRes,rankRes]=await Promise.all([authToken?apiFetch('/api/leaderboard'):fetch('/api/leaderboard'),authToken?apiFetch('/api/leaderboard/rank'):Promise.resolve(null)]);if(!lbRes||!lbRes.ok)return;const data=await lbRes.json();const my=stats.w*100+stats.s*50;let all=data.map(r=>({n:r.username||'Player',s:(r.wins||0)*100+(r.streak||0)*50,me:!!r.is_me}));if(!all.some(p=>p.me))all.push({n:'YOU',s:my,me:true});all.sort((a,b)=>b.s-a.s);const top8=all.slice(0,8);const myInTop=top8.some(p=>p.me);let html=top8.map((p,i)=>`<div class="lb-item ${p.me?'me':''}"><div class="lb-rank ${i<1?'g':i<2?'s':i<3?'b':'n'}">${i+1}</div><div class="lb-name">${p.n}</div><div class="lb-score">${p.s}</div></div>`).join('');if(!myInTop&&rankRes&&rankRes.ok){const{rank}=await rankRes.json();html+=`<div class="lb-my-rank">📍 Your rank: <strong style="color:var(--green)">#${rank}</strong><span style="margin-left:auto">${my} pts</span></div>`;}document.getElementById('lb-list').innerHTML=html;}catch(e){console.warn('Leaderboard load failed',e);}}
...
function showPage(id){if(!_pages.includes(id))id='home';document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-'+id).classList.add('active');if(id==='profile'){updateProf();if(authToken)loadProfFromDB();track('profile_view');}if(id==='home')updateGameCards();track('page_view',{page:id});window.scrollTo(0,0);history.replaceState(null,'','#'+id);}
...
function shareIG(){track('share',{game:lastG,method:'instagram'});const c=document.createElement('canvas');c.width=400;c.height=500;const x=c.getContext('2d');const gr=x.createLinearGradient(0,0,400,500);gr.addColorStop(0,'#0a0f1a');gr.addColorStop(1,'#1a2332');x.fillStyle=gr;x.fillRect(0,0,400,500);x.strokeStyle='#10b981';x.lineWidth=4;x.beginPath();x.roundRect(10,10,380,480,16);x.stroke();x.fillStyle='#10b981';x.font='20px Outfit';x.textAlign='center';x.fillText(lastG,200,80);x.fillStyle='#f1f5f9';x.font='30px Bebas Neue';x.fillText('MY RESULT',200,140);x.fillStyle='#10b981';x.font='bold 100px Bebas Neue';x.fillText(lastS,200,280);x.fillStyle='#64748b';x.font='18px Outfit';x.fillText('⚽ footio.online',200,450);const a=document.createElement('a');a.download='footio.png';a.href=c.toDataURL();a.click();showToast('Image downloaded! Add to Instagram Stories.','ok')}
// Legacy
let legG={c:[],f:{},sl:0};function startLeg(){if(!checkPlayable('legacy'))return;document.getElementById('leg-i').style.display='none';document.getElementById('leg-g').style.display='block';_gameStart=Date.now();track('game_started',{game:'legacy'});initLeg()}function resetLeg(){if(isPlayedToday('legacy')){showToast('You already played this game today! Come back tomorrow 🌅','info');showPage('home');return}initLeg()}
...
// Grid
let gridG={r:[],cl:[],ce:Array(9).fill(null),gu:9,ac:null};function startGrid(){if(!checkPlayable('grid'))return;document.getElementById('grid-i').style.display='none';document.getElementById('grid-g').style.display='block';_gameStart=Date.now();track('game_started',{game:'grid'});initGrid()}function resetGrid(){if(isPlayedToday('grid')){showToast('You already played this game today! Come back tomorrow 🌅','info');showPage('home');return}initGrid()}
...
// Wordle
let wordG={t:null,g:[],m:8};function startWord(){if(!checkPlayable('wordle'))return;document.getElementById('word-i').style.display='none';document.getElementById('word-g').style.display='block';_gameStart=Date.now();track('game_started',{game:'wordle'});initWord()}function resetWord(){if(isPlayedToday('wordle')){showToast('You already played this game today! Come back tomorrow 🌅','info');showPage('home');return}initWord()}
...
function startPyr(){if(!checkPlayable('pyramid'))return;document.getElementById('pyr-i').style.display='none';document.getElementById('pyr-g').style.display='block';_gameStart=Date.now();track('game_started',{game:'pyramid'});initPyr()}function resetPyr(){if(isPlayedToday('pyramid')){showToast('You already played this game today! Come back tomorrow 🌅','info');showPage('home');return}initPyr()}
...
function startImp(){if(!checkPlayable('impostor'))return;document.getElementById('imp-i').style.display='none';document.getElementById('imp-g').style.display='block';_gameStart=Date.now();track('game_started',{game:'impostor'});initImp()}function resetImp(){if(isPlayedToday('impostor')){showToast('You already played this game today! Come back tomorrow 🌅','info');showPage('home');return}initImp()}
...
async function initAuth(){
  if(authToken){
    try{
      ...
      if(res&&res.ok){const d=await res.json();authUser={email:d.email,id:d.id};await loadProfFromDB();}
...
async function authVerifyCode(){
  ...
  try{
    ...
    if(res.ok){
      const data=await res.json();
      authToken=data.token;authUser={email:data.email,id:data.id};track('auth_completed');
