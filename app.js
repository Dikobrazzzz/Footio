...
async function loadLeaderboard(){try{const [res,rankRes]=await Promise.all([fetch('/api/leaderboard'),authToken?apiFetch('/api/leaderboard/rank'):Promise.resolve(null)]);if(!res.ok)return;const data=await res.json();const my=stats.w*100+stats.s*50;let all=data.map(r=>({n:r.username||'Player',s:(r.wins||0)*100+(r.streak||0)*50}));all.push({n:'YOU',s:my,me:1});all.sort((a,b)=>b.s-a.s);const top8=all.slice(0,8);const myInTop=top8.some(p=>p.me);let html=top8.map((p,i)=>`<div class="lb-item ${p.me?'me':''}"><div class="lb-rank ${i<1?'g':i<2?'s':i<3?'b':'n'}">${i+1}</div><div class="lb-name">${p.n}</div><div class="lb-score">${p.s}</div></div>`).join('');if(!myInTop&&rankRes&&rankRes.ok){const{rank}=await rankRes.json();html+=`<div class="lb-my-rank">📍 Your rank: <strong style="color:var(--green)">#${rank}</strong><span style="margin-left:auto">${my} pts</span></div>`;}document.getElementById('lb-list').innerHTML=html;}catch(e){console.warn('Leaderboard load failed',e);}}
