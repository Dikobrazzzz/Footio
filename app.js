...
async function loadLeaderboard(){try{const res=await fetch('/api/leaderboard');if(!res.ok)return;const data=await res.json();const my=stats.w*100+stats.s*50;let all=data.map(r=>({n:r.username||'Player',s:(r.wins||0)*100+(r.streak||0)*50}));all.push({n:'YOU',s:my,me:1});all.sort((a,b)=>b.s-a.s);document.getElementById('lb-list').innerHTML=all.slice(0,8).map((p,i)=>`<div class="lb-item ${p.me?'me':''}"><div class="lb-rank ${i<1?'g':i<2?'s':i<3?'b':'n'}">${i+1}</div><div class="lb-name">${p.n}</div><div class="lb-score">${p.s}</div></div>`).join('');}catch(e){console.warn('Leaderboard load failed',e);}}
function updateGameCards(){const games=['legacy','grid','wordle','goltexto','pyramid','impostor'];games.forEach(g=>{const card=document.querySelector(`[onclick="showPage('${g}')"]`);if(card&&isPlayedToday(g)){card.style.opacity='0.5';card.style.pointerEvents='none';const playBtn=card.querySelector('.play-btn');if(playBtn)playBtn.textContent='PLAYED ✓'}})}
